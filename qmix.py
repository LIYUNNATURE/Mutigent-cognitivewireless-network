...
class QMIX:
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        s, s_next, u, r, avail_u, avail_u_next = batch['s'], batch['s_next'], batch['u'], \
                                                             batch['r'],  batch['avail_u'], batch['avail_u_next']
                                                            #  batch['terminated']
        mask = 1 - batch["padded"].float()  # 用来把那些填充的经验的TD-error置0，从而不让它们影响到学习

        # print('qmix-batch:',batch)
        # print('max_episode_len:',max_episode_len)
        ...
        self.optimizer.zero_grad() #将梯度的置零。也就是把loss关于weight导数变为零
        loss.backward()
    ...
    def _get_inputs(self, batch, transition_idx):
        ...
        # print('qminx-inputs:',inputs)
        # print('qmix-inputs_next:',inputs_next)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.eval_qmix_net.state_dict(), self.model_dir + '/qmix_net_params.pkl')#self.model_dir + '/' + num + '_qmix_net_params.pkl')
        torch.save(self.eval_rnn.state_dict(),  self.model_dir + '/rnn_net_params.pkl')#self.model_dir + '/' + num + '_rnn_net_params.pkl')
